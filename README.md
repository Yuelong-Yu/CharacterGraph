...
- 自动巡游：默认按高度数节点优先的 BFS 序列巡游，也可从按钮菜单切换为 DFS。
...
共享注册 contract 与表单字段，并通过
`chronchaos_gpt/packages/top-navigation` 完整复用 ChronChaos 顶部导航。
请将两个仓库分别以 `chronchaos_gpt/` 和 `CharacterGraph/` 的名称放在
同一父目录下，并先安装 `chronchaos_gpt`。
