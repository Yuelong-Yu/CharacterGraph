...
SYSTEM = """你是希腊神话古典文献译者。任务：把英文/希腊文的引语翻译为优美、忠实于原意的中文。

原则：
1. 保留古典文献的庄重感和诗意，不要现代口语化。
2. 保留人名、地名的标准中文译名（如 Achilleus→阿喀琉斯，Argos→阿尔戈斯）。
...
def translate(slug: str, name: str, text: str, source: dict) -> str:
...
def main():
    for path in sorted(CHAR_DIR.glob("*.json")):
        ...
        for q in data["quotes"]:
            ...
            try:
                new_text = translate(data["id"], data["name_zh"], text, q.get("source", {}))
                print(f"   → {new_text[:60]}...")
                q["text"] = new_text
                dirty = True
